...
enough for that to matter; the magnitudes are.
    ...
    python scripts/er_01_diagnose.py                # all three
    python scripts/er_01_diagnose.py --skip-monte-carlo
"""
...
def main() -> None:
    ...
    if not glob.glob(args.runs_glob):
        raise SystemExit(
            f"no runs matched {args.runs_glob}; run scripts/02_estimate_models.py first"
        )
