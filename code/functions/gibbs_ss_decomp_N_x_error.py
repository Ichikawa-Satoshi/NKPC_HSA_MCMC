...
def estimate_hsa(pi_data: np.ndarray, pi_prev_data: np.ndarray, Epi_data: np.ndarray,
                 ...
                 include_x_measurement_error: bool = False) -> Dict[str, Any]:
    if seed is not None:
        np.random.seed(seed)
    ...
    default_priors = {
        ...
        "mu_kappa": 0.0, "sigma_kappa": 0.01,
        "mu_theta": 0.0, "sigma_theta": 0.01,
    ...
    # Storage arrays - define n_store first
    ...
    if include_x_measurement_error:
        sigma_e2_x = 0.01  # x_tのmeasurement error分散
        sigma_prior2_x = 1.0  # x_trueの事前分散
        x_true = x_t.copy()  # 初期値
        # 事前分布パラメータ
        a_e_x = priors["a_e_x"]
        b_e_x = priors["b_e_x"]
        # 保存用配列
        sigma_e2_x_draws = np.zeros(n_keep)
        x_true_draws = np.zeros((n_store, T))
    else:
        x_true = x_t  # Use x_t directly

    print(f"Starting Gibbs sampling: burn-in={n_burn}, keep={n_keep}")
    for iter_ in trange(1, n_burn + n_keep + 1, desc="Gibbs sampling"):
        if include_x_measurement_error:
            # ---- Sample x_true (白色雑音) ----
            # x_true_t ~ N(0, sigma_prior2_x) (事前分布)
            # x_obs_t = x_true_t + e_t,  e_t ~ N(0, sigma_e2_x)
            # 事後分布: x_true_t | x_obs_t ~ N(post_mean_t, post_var_t)
            
            post_precision = 1.0/sigma_prior2_x + 1.0/sigma_e2_x
            post_var = 1.0 / post_precision
            
            for t in range(T):
                post_mean = (x_t[t] / sigma_e2_x) * post_var
                x_true[t] = post_mean + np.sqrt(post_var) * np.random.randn()
            
            # ---- Sample sigma_e2_x (measurement error variance) ----
            e_resid = x_t - x_true
            a_post_e_x = a_e_x + T / 2.0
            b_post_e_x = b_e_x + 0.5 * np.sum(e_resid ** 2)
            sigma_e2_x = 1.0 / np.random.gamma(shape=a_post_e_x, scale=1.0 / b_post_e_x)

        # ---- Sample alpha ----
        y_alpha = pi_t - E_pi_tp1 - kappa * x_true + theta * Nhat
        ...
        # ---- Sample theta ----
        y_theta = pi_t - alpha * pi_tm1 - (1.0 - alpha) * E_pi_tp1 - kappa * x_true
        X_theta = -Nhat
        XtX = X_theta @ X_theta
        if abs(XtX) > 1e-12:
        ...
        nkpc_resid = pi_t - alpha * pi_tm1 - (1.0 - alpha) * E_pi_tp1 - kappa * x_true + theta * Nhat
        a_post_v = a_v + T / 2.0
        ...
        Nhat = sample_ar2_states_ffbs(N_data - Nbar, rho1, rho2, sigma_eps2,
                                      pi_t, alpha, pi_tm1, E_pi_tp1, kappa, x_true, theta, sigma_v2)
        ...
        if iter_ > n_burn:
            ...
            if n_store > 0 and (idx % 10 == 0) and (idx // 10 < n_store):
                ...
                if include_x_measurement_error:
                    sigma_e2_x_draws[idx] = sigma_e2_x
                    x_true_draws[store_idx, :] = x_true
    ...
    # States
    if n_store > 0:
        actual_stores = min(n_store, n_keep // 10 if n_keep // 10 > 0 else n_store)
        Nbar_mean = np.mean(Nbar_draws[:actual_stores, :], axis=0)
        Nhat_mean = np.mean(Nhat_draws[:actual_stores, :], axis=0)
    else:
        Nbar_mean = Nbar
        Nhat_mean = Nhat
    results["states"] = {
    ...
    if include_x_measurement_error:
        results["sigma_e2_x"] = summarize(sigma_e2_x_draws)
        if n_store > 0:
            actual_stores = min(n_store, n_keep // 10)
            x_true_mean = np.mean(x_true_draws[:actual_stores, :], axis=0)
            results["x_true"] = {"x_true_mean": x_true_mean}
