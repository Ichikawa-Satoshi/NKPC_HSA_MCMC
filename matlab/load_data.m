...
% === Data preparation ===
%% CPI
data_cpi = readtable("../data/CPIAUCSL.csv");
...
data_cpi.pi = pi_yoy;
%% HHI
data_HHI_annual = readtable("../data/hhi_from_Gustavo_2019.csv");
...
%% Markup
data_markup = readtable("../data/nekarda_ramey_markups.xlsx");
data_markup.DATE = datetime(data_markup.qdate,'InputFormat','yyyy-MM-dd');
data_markup.markup = data_markup.mu_bus;
data_markup = data_markup(:,{'DATE','markup'});
data_markup = rmmissing(data_markup);
%% De-trended Markup (BN filter)
data_markup_BN = readtable("../data/BN_markup_inv.csv");
...
data_output = readtable("../data/BN_filter_GDPC1_quaterly.csv");
...
data_Epi = readtable("../data/Epi.xlsx");
...
data = outerjoin(data,data_markup,'Keys','DATE','MergeKeys',true);
