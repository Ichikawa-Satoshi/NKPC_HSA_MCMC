...
error down to $\sigma_N\approx0.023$ (versus $0.068$ under mixed frequency), and through
...
between the two states---posterior correlation $-0.9996$ under interpolation against
$+0.13$ under mixed frequency---which is the identification pathology described in
Section~\ref{sec:scope}. The interpolated design also forces the AR(2) cycle to a
near-unit root and hence to an $82$-quarter ``cycle'' that is a second trend rather than a
...
HSA full       & $\kappa_0+\delta\bar N_t$ & $\theta_0+\gamma\bar N_t$ & HSA Full & Gibbs + Particle Gibbs\\
\bottomrule
\end{tabular}
\end{table}
...
worst-mixing scalar is the trend drift $n$ in 35 of the 38 failing cells---the level ridge
of Section~\ref{sec:scope} surfacing in the parameter block. Under mixed frequency it is
the AR(2) block $(\rho_1,\rho_2)$ in 53 of 61 cells, which for HSA steady is a nuisance
object because $\theta=0$ keeps $\hat N_t$ out of the inflation equation. The parameters
the conclusions rest on fail at the \emph{same} rate in the two designs: $\delta$ in 8 of
61 cells and $\kappa_0$ in 6 of 61, either way. The $95\%$ credible intervals are the $2.5\%$ and $97.5\%$
...
\input{results/tables/cpi_ppi_report/annual_q4/headline_results.tex}
...
under mixed frequency that is the AR(2) block $(\rho_1,\rho_2)$ in 53 of the 61 HSA cells.
...
$\hat R=1.000$ with bulk ESS between $2{,}600$ and $3{,}400$ in eight of the nine cells.
...
\input{results/tables/cpi_ppi_report/annual_q4/model_comparison_unemp.tex}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.62\linewidth]{cpi_ppi_report/annual_q4/kappa_t_unemployment_by_inflation.png}
...
$0.11$ at the beginning of the sample to approximately zero by the end. This change is
economically meaningful. For example, a four-point deterioration in the negative
unemployment gap ($x_t=-4$) implies an inflation response of about $-0.44$ percentage
...
\input{results/tables/cpi_ppi_report/annual_q4/fit_comparison.tex}

For HSA steady against CES the plug-in score gives $+7.93$ on core CPI where
$\mathrm{LPD}_1$ gives $+3.92$ and WAIC and LOO give $+3.29$ and $+3.28$; on headline CPI
it gives $+4.56$ against $+2.75$, $+0.89$ and $+0.83$. The plug-in score overstates the
improvement by a factor of roughly two to five, which is what one expects from a
...
improves every score, on PPI no score separates the models (LPD$_1$ $+0.40$, WAIC $-0.54$,
LOO $-0.92$), and HSA dynamic---the entry channel without a time-varying slope---never
beats HSA steady. Two caveats. First, the LOO column is unreliable wherever the maximum Pareto
$\hat k$ exceeds $0.7$, which is 5 of the 12 cells: CES and HSA full on core CPI
($0.84$ and $0.92$), and CES, HSA steady and HSA dynamic on PPI ($1.00$, $1.25$, $1.09$).
That includes the CES baseline for the core-CPI comparison, so the $+3.28$ LOO difference
quoted above rests on one number that PSIS itself flags; the WAIC difference of $+3.29$,
...
measuring the posterior. For the main cell the kernel estimate moves from $22$ to
$6.6\times10^{4}$ as its bandwidth is scaled between $3\times$ and $0.5\times$ Scott's
rule, whereas \eqref{eq:sddr-rb} draws on $1{,}504$ effectively contributing terms out
of $3{,}200$ (the three largest carry $1.2\%$ of the sum) and has a Monte Carlo standard
error of about $3\%$.
...
\midrule
Negative unemployment gap & \AnnualHeadlineUnempDelta\ (\AnnualHeadlineUnempDeltaBF) & \AnnualCoreUnempDelta\ (\AnnualCoreUnempDeltaBF) & \AnnualPPIUnempDelta\ (\AnnualPPIUnempDeltaBF) \\
Output gap (HP)  & \AnnualHeadlineHPDelta\ (\AnnualHeadlineHPDeltaBF) & \AnnualCoreHPDelta\ (\AnnualCoreHPDeltaBF) & \AnnualPPIHPDelta\ (\AnnualPPIHPDeltaBF) \\
Output gap (BN)  & \AnnualHeadlineBNDelta\ (\AnnualHeadlineBNDeltaBF) & \AnnualCoreBNDelta\ (\AnnualCoreBNDeltaBF) & \AnnualPPIBNDelta\ (\AnnualPPIBNDeltaBF) \\
\bottomrule
\end{tabular}
\end{table}
...
Estimated measurement error $\sigma_N$  & $0.068$   & $0.023$ \\
$\rho_1,\rho_2$                         & $+0.20,\,-0.89$ & $+1.80,\,-0.81$ \\
Implied AR(2) period                    & $4.3$ quarters & $82$ quarters \\
Mean reversion $1-\rho_1-\rho_2$        & $1.69$    & $0.014$ \\
Posterior corr$(\bar N_0,\hat N_0)$     & $+0.13$   & $\mathbf{-0.9996}$ \\
Posterior corr$(\bar N$ level$,\kappa_0)$ & $-0.32$ & $-0.55$ \\
\midrule
$\delta$                                & \AnnualCoreUnempDelta & \CoreUnempDelta \\
$\kt$ start $\to$ end                   & $\AnnualCoreUnempKappaStart \to \AnnualCoreUnempKappaEnd$
                                        & $\CoreUnempKappaStart \to \CoreUnempKappaEnd$ \\
\bottomrule
\end{tabular}
\end{table}

Three things follow. First, the interpolated series is so smooth that the model drives
$\sigma_N$ down to $0.023$ to fit it, which pins $\bar N_t+\hat N_t$ at all 124 quarters
and leaves only the split free. Second, the AR(2) cycle is pushed to a near-unit root,
$\rho_1+\rho_2=0.986$, so its mean-reversion term $1-\rho_1-\rho_2$ collapses to $0.014$.
...
counter-identified, which is the $-0.9996$ in the table. Across all 122 estimated HSA
cells the correlation between $1-\rho_1-\rho_2$ and corr$(\bar N_0,\hat N_0)$ is $+0.92$,
so this is the mechanism and not a coincidence. Third, the ``cycle'' the interpolated
design recovers has an $82$-quarter period: it is a second trend competing with $\bar N_t$
...
\input{results/tables/cpi_ppi_report/headline_results.tex}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.88\linewidth]{cpi_ppi_report/competition_decomposition_hsa_steady_core_unemployment.png}
...
\tiny
\resizebox{\linewidth}{!}{\input{results/tables/cpi_ppi_report/group_convergence_diagnostics.tex}}
\end{table}
...
under the exact joint FFBS 6 of its 15 cells now pass, including both CPI
negative-unemployment-gap cells. Its $\delta$ agrees in sign and magnitude with HSA
steady there. That is a computational fix, not new identification.
...
concentrated in the AR(2) block: $(\rho_1,\rho_2)$ is the worst-mixing scalar in 53 of
the 61 HSA cells, and the latent-state paths fail alongside it. What does \emph{not} fail
is the coefficient the conclusions rest on: $\delta$ has $\hat R=1.000$ and bulk ESS above
$2{,}600$ in eight of the nine HSA-steady baseline cells, and $\delta$ and $\kappa_0$
fail at exactly the same rate under either observation design (8 and 6 cells out of 61).
...
the AR(2) is pushed to a near-unit root and $1-\rho_1-\rho_2$ collapses to $0.014$, so the
anchor effectively disappears and the posterior correlation between the two states reaches
$-0.9996$. Under the mixed-frequency design used here the same quantity is $1.69$ and the
correlation is $+0.13$. Across all 122 estimated HSA cells the two quantities correlate at
$+0.92$, which identifies the mechanism (Section~\ref{sec:pchip}).
...
the posterior correlation between the $\bar N$ level and $\kappa_0$ is $-0.55$ there
against $-0.32$ here. Statements about the \emph{level} of the Phillips-curve slope should
...
$+0.038\,[+0.013,+0.124]$ (weak), $+0.022\,[+0.008,+0.038]$ (baseline),
$+0.009\,[-0.007,+0.024]$ (tight), so the tight prior pulls the interval across zero. The
...
to see which channel drives the sweep. The answer is $\delta$'s own prior: tightening it
alone accounts for $84\%$ of the headline-CPI shrinkage and $78\%$ of the core-CPI
shrinkage, against $13\%$ and $50\%$ for the AR(2) prior (with a $-28\%$ interaction in
the core cell). This matters because the AR(2) prior is in tension with the interpolated
data (Section~\ref{sec:pchip}); the experiment shows that tension is not what is moving
$\delta$. It does, however, break the sampler in the tight corner: the cells where the
AR(2) prior fights the data hardest are where $\delta$'s own ESS collapses.

\paragraph{Model variants.} HSA const-theta reproduces the main result almost exactly
under the added entry term: $\delta=+0.029\,[+0.003,+0.055]$ for headline CPI and
$\delta=+0.022\,[+0.004,+0.038]$ for core CPI, against $+0.029$ and $+0.022$ under HSA
...
comparison also holds, and there 6 of the 15 const-theta cells additionally satisfy the
coefficient rule; under mixed frequency none do, for the AR(2) reason above. HSA full
...
negative unemployment gap differs by about $0.11$ point.
...
\item \textbf{Cyclical-entry channel unconfirmed, and sign-unstable.} The theory
predicts a positive entry coefficient $\theta$ in $-\theta_t\hat N_t$. In the main design
HSA const-theta gives $+0.022\,[-0.159,+0.241]$ for headline CPI and
$-0.039\,[-0.136,+0.054]$ for core CPI---opposite signs across two price indices, both
intervals wide and straddling zero. The sign is not stable across the observation design
either: the same core-CPI cell gives $+0.040$ with $\Pr(\theta>0)=0.89$ under
interpolation against $-0.039$ with $\Pr(\theta>0)=0.15$ here. Under interpolation the
sign also flips between activity measures ($+0.040$ core CPI with the unemployment gap
versus $-0.049$ with the BN output gap, the latter putting $97\%$ of its mass on the
theory-wrong side). We claim no support for this channel and note that what the data say
about its sign depends on choices that should not matter. $\gamma$ is estimated only in
...
same number. What the interpolation changes is everything around it: it drives the
firm-count measurement error to a third of its mixed-frequency value, forces the AR(2)
cycle to a near-unit root, and through that opens a near-exact ridge between the trend and
cycle states (posterior correlation $-0.9996$ against $+0.13$). We report it in
...
\tiny
\resizebox{\linewidth}{!}{\input{results/tables/cpi_ppi_report/annual_q4/group_convergence_diagnostics.tex}}
\end{table}
