...
(annual), i.e.\ the inverse of the \emph{HHI of U.S.\ listed firms}. The
...
\begin{tabular}{lllll}
\toprule
Model & $\kt$ & $\theta_t$ & Role & State block \\
\midrule
CES             & constant & $0$ & Textbook NKPC benchmark & Gibbs (conjugate)\\
HSA steady     & $\kappa_0+\delta\bar N_t$ & $0$ & $\bar N$ channel only  & Gibbs + exact joint FFBS\\
HSA dynamic    & constant & constant & $\hat N$ channel only  & Gibbs + exact joint FFBS\\
HSA const-theta & $\kappa_0+\delta\bar N_t$ & constant & HSA full with $\gamma=0$ & Gibbs + exact joint FFBS\\
HSA full       & $\kappa_0+\delta\bar N_t$ & $\theta_0+\gamma\bar N_t$ & HSA Full & Gibbs + Particle Gibbs (PCHIP)\\
\bottomrule
\end{tabular}
\end{table}
...
path (512 particles). This applies to the PCHIP cells reported in the body and
appendices; the annual-Q4 HSA-full cells of Section~\ref{sec:annual-q4-results} are
\emph{not} re-estimated by Particle Gibbs and still use the two alternating conditional
FFBS blocks, which is why their diagnostics are reported separately.
...
correlated in the posterior (about $-0.999$: the firm-count equation pins their sum but
...
Of the \ReportRunCount{} PCHIP runs, \ReportWarningCount{} fail the scalar rule,
\ReportStateWarningCount{} fail the state-path rule and \ReportJointWarningCount{} fail at
least one of the three. Note that the scalar rule covers the trend drift $n$ and the
variance block, which earlier revisions of this table omitted; $n$ is the worst-mixing
scalar in every HSA cell, which is the level-ridge of Section~\ref{sec:scope} showing up
in the parameter block. The $95\%$ credible intervals are the $2.5\%$ and $97.5\%$
...
main HSA steady (negative unemployment gap, core CPI). Lines are posterior means, bands are
...
\label{sec:main}
Table~\ref{tab:headline} gives the main-model (HSA steady) estimates for every price
index and activity measure, so each result can be read against the specification it comes
from. The main specification---\textbf{core CPI, negative unemployment gap}---is in bold;
a positive $\delta$ means the slope flattens as the firm count falls.
\input{results/tables/cpi_ppi_report_en/headline_results.tex}
...
\input{results/tables/cpi_ppi_report_en/model_comparison_unemp.tex}
...
result for the negative unemployment gap is robust to the inflation index. Figure~\ref{fig:kt_unemp}
...
\caption{Time-varying slope $\kt$ for the negative unemployment gap (posterior mean and $95\%$
band). Under CPI it declines to near zero by the end of the sample, whereas the PPI
band is wide.}
\label{fig:kt_unemp}
\end{figure}

The estimated Phillips-curve slope in the core-CPI specification declines from about
0.12 at the beginning of the sample to approximately zero by the end of the sample.
This change is economically meaningful. For example, a four-point deterioration in
the negative unemployment gap ($x_t=-4$) implies an inflation response of about
$-0.48$ percentage point at the 1982 slope, but almost no response at the 2012 slope.
...
\label{sec:mc}
We compare CES (constant $\kappa$) against HSA steady (slope varying with the
...
density) is left for future work. In the negative-unemployment-gap specification the time-varying
...
\midrule
Negative unemployment gap & \HeadlineUnempDelta\ (\HeadlineUnempDeltaBF) & \CoreUnempDelta\ (\CoreUnempDeltaBF) & \PPIUnempDelta\ (\PPIUnempDeltaBF) \\
...
more complex model. A corrected conditional marginal likelihood
$p(\pi\mid x,N^{\mathrm{obs}},M)$---states integrated out exactly by the Kalman filter,
Chib ordinate with explicit reduced Gibbs runs---now exists in the repository
(Section~\ref{sec:status}), but its numbers are not promoted into this table until they
have been reviewed.
...
HSA steady. For the negative unemployment gap, headline CPI gives
...
indices of the negative unemployment gap, the maximum $\hat R$ of $\delta$ is 1.000 throughout,
...
HSA steady, negative unemployment gap. Rhat is the maximum along the path, ESS the minimum bulk
...
\caption{Annual-Q4 version: time-varying slope $\kappa_t$ for the negative unemployment gap
...
For core CPI and the negative unemployment gap the implied slope moves from
...
\caption{Annual-Q4 version: firm-count state decomposition for core CPI, negative unemployment
...
For the negative unemployment gap we show the same five models as the PCHIP version by price
...
\begin{table}[H]
\centering
\caption{Annual-Q4 version: model-by-model results for the negative unemployment gap. CES is a
...
\caption{Annual-Q4 version: slope at average competition for the negative unemployment gap.
...
\begin{table}[H]
\centering
\caption{Annual-Q4 version: all models, three-prior sensitivity for the negative unemployment
...
Also, because the $\delta$ interval for the CPI negative-unemployment-gap specification includes zero
...
\begin{table}[H]
\centering
\caption{Three-prior sensitivity of the current results (negative-unemployment-gap
...
negative unemployment gap differs by about 0.126 point.
...
\item \textbf{Cyclical-entry channel unconfirmed.} Many HSA const-theta/full
...
This draft uses the \ReportRunCount{} runs of the current corrected estimation revision.
...
for the CPI negative-unemployment-gap specification of HSA steady; the positive sign of the core-CPI
...
\begin{itemize}
\item \textbf{Conditional marginal likelihood.} A corrected implementation of
$p(\pi\mid x,N^{\mathrm{obs}},M)$ now exists: the latent states are integrated out
exactly by the Kalman filter through
$\log p(\pi\mid N^{\mathrm{obs}},\theta)=\log p(\pi,N^{\mathrm{obs}}\mid\theta)-\log
p(N^{\mathrm{obs}}\mid\theta)$, the filter uses the estimating model's own initial state
(earlier versions started the trend at $N^{\mathrm{obs}}_0$, evaluating a different model
and using the first observation twice), the AR(2) ordinate is the sampler's own
initial-lag conditional, the truncated $(\rho_1,\rho_2)$ prior and conditional are
normalized over the stationary triangle, and the ordinate uses Chib's sequential
factorization with explicit reduced Gibbs runs. HSA full is excluded by construction:
its bilinear latent term is not linear-Gaussian, and the previous ``conditional'' figure
for it conditioned on posterior-\emph{mean} states, which is a plug-in and not a marginal
likelihood. These numbers are computed but not yet promoted into any table.
...
near $-0.999$; only their sum is pinned by the firm-count equation. $\kappa_0$ is
...
ESS $\geq400$. This table is the evidence behind the $\dagger$ and ``OK (coef)'' marks
elsewhere: it shows whether a failure is driven by a coefficient of interest or by a
nuisance term.}
\label{tab:group-convergence}
\tiny
\resizebox{\linewidth}{!}{\input{results/tables/cpi_ppi_report_en/group_convergence_diagnostics.tex}}
\end{table}
