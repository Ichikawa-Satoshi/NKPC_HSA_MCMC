function [results, models] = func_ols_hsa_system(data, pi, Epi, pi_prev, Nhat, oil, IncludeOil)
%FUNC_OLS_HSA_SYSTEM Run OLS specs for inflation (optionally include oil).
%   [RESULTS, MODELS] = func_ols_hsa_system(DATA, pi, Epi, pi_prev, Nhat, IncludeOil)
    % ----- candidate x's -----
    specs = {
    ...
    % ----- base table -----
    if IncludeOil
        baseTbl = table(pi, Epi, pi_prev, Nhat, oil, ...
            'VariableNames', {'pi','Epi','pi_prev','Nhat','oil'});
    else
        baseTbl = table(pi, Epi, pi_prev, Nhat, ...
            'VariableNames', {'pi','Epi','pi_prev','Nhat'});
    end
    ...
    for i = 1:size(specs,1)
        ...
        T = rmmissing(T);
        formula = "pi ~ -1 + pi_prev + Epi + x + Nhat";
        if IncludeOil
            formula = formula + " + oil";
        end
        mdl = fitlm(T, formula);
