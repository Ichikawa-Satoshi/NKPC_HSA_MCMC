...
% kappa_t = kappa_0 + delta * (Nbar_t - Nbar_0)   [DETERMINISTIC]
    ...
    for iter = 1:total_iter
        ...
        % ===== 1. alpha | . =====
        % Model: pi_t - E_t pi_{t+1} - kappa_t x_t + theta Nhat_t = alpha * (pi_{t-1} - E_t pi_{t+1}) + eps
        
        % [CORRECTION 1] Signs flipped for kappa_t*x_t and theta*Nhat when moved to LHS
        y_a = pi_t - E_pi_tp1 - kappa_t.*x_t + theta.*Nhat; % <--- FIXED
        X_a = pi_tm1 - E_pi_tp1;
        ...
        % ===== 2. theta | . =====
        % Model: pi_t - alpha*pi_{t-1} - (1-alpha)E_t pi_{t+1} - kappa_t x_t = -theta * Nhat_t + eps
        
        % [CORRECTION 2] Sign flipped for kappa_t*x_t when moved to LHS
        y_th = pi_t - alpha.*pi_tm1 - (1-alpha).*E_pi_tp1 - kappa_t.*x_t; % <--- FIXED
        X_th = -Nhat; % Regressor is -Nhat because term is "- theta * Nhat"
        ...
        % (Note: +theta*Nhat because original term is -theta*Nhat, moved to LHS it becomes +)
        resid = pi_t - alpha.*pi_tm1 - (1-alpha).*E_pi_tp1 + theta.*Nhat; % This was CORRECT in original
        
        % Linear regression: resid = kappa_t * x_t + noise
        %                           = (kappa0 + delta*dNbar_cum) * x_t
        ...
        % ===== 4. sigma_v2 | . =====
        kappa_t = kappa0 + delta * dNbar_cum; % Update kappa_t with new params
        
        % [CORRECTION 3] Residual calculation signs
        % eps = pi_t - (alpha*pi_{t-1} + (1-alpha)E - theta*Nhat + kappa*x)
        %     = pi_t - alpha*pi_{t-1} - (1-alpha)E + theta*Nhat - kappa*x
        eps = pi_t - alpha.*pi_tm1 - (1-alpha).*E_pi_tp1 - kappa_t.*x_t + theta.*Nhat; % <--- FIXED
