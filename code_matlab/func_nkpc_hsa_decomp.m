function results = func_nkpc_hsa_decomp(pi_data, pi_prev_data, Epi_data, x_data, N_data, n_burn, n_keep, priors, opts)
...
%
% Notes:
% - InvGamma is parameterized as shape-scale; MATLAB gamrnd(shape,scale).
    ...
    assertallpos([sigma_alpha,sigma_kappa,sigma_theta,sigma_rho1,sigma_rho2,sigma_n,a_v,b_v,a_eps,b_eps,a_eta,b_eta], ...
    ...
    for iter = 1:total_iter
        ...
        if T>=2
            ...
            tries = 0;
            while n_drift <= 0 && tries < 10000
                n_drift = postM + sqrt(postV) * randn;
                tries = tries + 1;
            end
            if n_drift <= 0
                % fallback: enforce positivity deterministically
                n_drift = abs(postM);
            end
        end
        ...
        if verbose && mod(iter, 5000)==0
            fprintf('Iter %d/%d: a=%.3f k=%.3f th=%.3f r1=%.3f r2=%.3f\n', ...
                iter, total_iter, alpha, kappa, theta, rho1, rho2);
        end
    end
