...
%% π and Eπ
axTop = nexttile(tl,1);
plot(axTop, data.DATE, data.pi_cpi, 'r-', 'LineWidth',2, 'DisplayName', 'Inflation \pi_t'); hold(axTop,'on');
...
tl2 = tiledlayout(tl,2,3,'TileSpacing','compact','Padding','compact');
tl2.Layout.Tile = 2;
...
%%
% (5) Inverse of HHI (BN, filled with cubic) and projected number of
% firm (BN)
ax5 = nexttile(tl2,5);
yyaxis(ax5,'left');
plot(ax5, data.DATE, data.N_BN, 'k-', 'LineWidth',2, 'DisplayName','inteporate');
ylabel(ax5,'inteporate','FontSize',12,'FontWeight','bold');
yyaxis(ax5,'right');
plot(ax5, data.DATE, data.N_p_BN, 'LineStyle','-.', 'Color',[0 0.6 0], 'LineWidth',2, 'DisplayName','projection');
ylabel(ax5,'projection','FontSize',12,'FontWeight','bold','Color',[0 0.6 0]);
xlabel(ax5,'Year'); title(ax5,'the number of firm (projection and inteporate)','FontSize',14);
grid(ax5,'on'); ax5.GridLineStyle='--'; ax5.GridAlpha=0.6;
