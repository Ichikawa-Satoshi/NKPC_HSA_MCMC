function [results, models] = func_ols_ces_system(data, pi, Epi, pi_prev)
...
%       3) markup_BN_inv
    ...
    results = table();
    models  = struct([]);
    baseTbl = table(pi, Epi, pi_prev, 'VariableNames', {'pi','Epi','pi_prev'});
    for i = 1:size(specs,1)
        label = specs{i,1};
        x     = specs{i,2};
        T = [baseTbl, table(x,'VariableNames',{'x'})];
        T = rmmissing(T);
        mdl = fitlm(T, "pi ~ -1 + pi_prev + Epi + x");
