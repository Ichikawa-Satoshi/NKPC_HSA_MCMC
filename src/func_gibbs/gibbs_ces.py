...
try:
    from .gibbs_utils import assert_all_pos, getd, sample_invgamma
...
def func_nkpc_ces(
    ...
    *,
    ...
    for it in range(1, total_iter + 1):
        ...
        prec_phi = phi_prec0 + float(np.sum(x_tm1**2)) / sigma_zeta2 + (lambda_ez**2) * float(np.sum(x_tm1**2)) / sigma_eta2
        mean_num_phi = (
            mu_phi * phi_prec0
            + float(np.dot(x_tm1, x_t)) / sigma_zeta2
            - lambda_ez * float(np.dot(x_tm1, y - alpha * a_t - kappa * x_t - lambda_ez * x_t)) / sigma_eta2
        )
        ...
        sigma_zeta2 = sample_invgamma(a_z + 0.5 * T, b_z + 0.5 * float(np.sum(zeta**2)), rng)
        sigma_eta2 = sample_invgamma(a_e + 0.5 * T, b_e + 0.5 * float(np.sum(eta**2)), rng)
