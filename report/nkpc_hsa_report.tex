...
First, ``weak'' is not a uniform widening: it multiplies most coefficient standard
deviations by $2.5$ but $\delta$ and $\gamma$ by $5$, and it \emph{recentres}
$\theta,\theta_0$ from $0.1$ to $0$, so the weak-prior entry-coefficient results mix a
dispersion change with a location change. Second, ``tight'' holds every variance prior
...
HSA const-theta gives $\AnnualConstThetaHeadlineTheta$ for headline CPI and
$\AnnualConstThetaCoreTheta$ for core CPI---opposite signs across two price indices, both
intervals wide and straddling zero. The sign is not stable across the observation design
either: the same core-CPI cell gives $\ConstThetaCoreTheta$ with
$\Pr(\theta>0)=\ConstThetaCoreThetaProb$ under
interpolation against $\AnnualConstThetaCoreTheta$ with
$\Pr(\theta>0)=\AnnualConstThetaCoreThetaProb$ here. Under interpolation the
sign also flips between activity measures ($\ConstThetaCoreTheta$ core CPI with the
unemployment gap
versus $\ConstThetaCoreBNTheta$ with the BN output gap, the latter putting
