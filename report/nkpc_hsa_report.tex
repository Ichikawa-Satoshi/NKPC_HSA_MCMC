...
% Sample metadata is generated from the selected main run. These fallbacks keep
% the source compilable before the deferred re-estimation creates the new macros;
% because the generated files are input first, their values take precedence.
\providecommand{\AnnualReportSampleStart}{1982Q1}
\providecommand{\AnnualReportSampleEnd}{2012Q4}
\providecommand{\AnnualReportSampleStartYear}{1982}
\providecommand{\AnnualReportSampleEndYear}{2012}
\providecommand{\AnnualReportObservationCount}{124}
\providecommand{\AnnualReportFirmObservationCount}{31}
\providecommand{\AnnualReportFirmMissingCount}{93}
...
\item apart from the hybrid-NKPC normalization that puts weights $\alpha$ and
$1-\alpha$ on lagged and expected inflation, we impose no structural
cross-coefficient restrictions from the HSA theory and approximate $\kt$ as a
\emph{linear} function of $\bar N_t$;
\item we add backward-looking inertia ($\alpha$), which is absent from \eqref{eq:fm};
\item expected inflation is the Cleveland Fed's model-based one-year expected-inflation
estimate (Section~\ref{sec:data}), not a direct survey forecast;
\end{itemize}

\section{Data}
\label{sec:data}
\paragraph{Sample.} Quarterly, \AnnualReportSampleStart{}--\AnnualReportSampleEnd{}
($T=\AnnualReportObservationCount$) in the US. These labels and the observation count
are read from the selected run's metadata when the report is rebuilt; they are not
maintained independently in the prose.

\paragraph{Inflation.} We compare headline CPI, core CPI, and PPI. Expected inflation $\mathbb{E}\pi$ is
the same Cleveland Fed model-based one-year expected-inflation series in all
specifications. It combines market and survey inputs but is not itself the Survey of
Professional Forecasters. The PPI specifications use the same expectation series as
CPI. The PPI estimates are therefore not a structural
estimate matched through the expectation term, but a semi-structural external-validity
check that changes both the observed inflation index and, relative to core CPI, the
inflation transformation (simple percentage change rather than log difference). A measurement mismatch with the
expectation series also remains for core CPI.

Headline CPI and PPI are simple four-quarter percentage changes; core CPI is a
four-quarter log difference. All three are sampled quarterly, so adjacent observations
overlap in three of four quarters. Consequently the backward-looking coefficient
$\alpha$ mixes genuine inertia with mechanical overlap, while the production likelihood
treats the remaining inflation disturbance as serially independent. The companion
\code{error\_robustness} implementation replaces it with an invertible MA(3), an order
motivated by the three-quarter overlap. Overlap alone does not mechanically imply that
the residual in this hybrid equation is MA(3); that requires an explicit aggregation
model for the underlying quarterly disturbance. Its full production sweep is deliberately not folded into the
headline tables until those runs have been completed; the current headline posterior is
therefore the i.i.d.-disturbance specification throughout, not a mixture of the two.
...
U.S.\ listed firms}. It is \textbf{annual}: over
\AnnualReportSampleStartYear{}--\AnnualReportSampleEndYear{} there are
\AnnualReportFirmObservationCount{} observations, not \AnnualReportObservationCount{}.

The main results therefore use a \emph{mixed-frequency} observation scheme. The annual
value enters the state-space model in that year's Q4 and the intervening
\AnnualReportFirmMissingCount{} quarters are
...
value is then used \emph{as data} in all \AnnualReportObservationCount{} quarters
(Section~\ref{sec:pchip}).
...
that pins $\bar N_t+\hat N_t$ so tightly at all \AnnualReportObservationCount{} quarters
that only the \emph{split}
...
\end{cases}
\qquad |\mathcal T_N|=\AnnualReportFirmObservationCount,
...
\eqref{eq:N-measurement} at all \AnnualReportObservationCount{} quarters;
Section~\ref{sec:pchip} reports it and
...
iteration. The inverse-gamma update of $\sigma_N^2$ likewise uses only the
\AnnualReportFirmObservationCount{} finite
Q4 residuals.
...
The $\lambda_{e\zeta}\sim N(0,0.5^2)$ prior and the initial-state prior are code-level
defaults shared by the baseline, weak, and tight sweeps; they are not varied by the
three YAML prior sets. Table~\ref{tab:baseline-priors} lists the former for completeness,
not because it is a baseline-only choice.
...
\label{sec:main}
All results in this section use the \textbf{mixed-frequency} firm-count design: the
\AnnualReportFirmObservationCount{} annual observations enter in their own Q4 and the
\AnnualReportFirmMissingCount{} intervening quarters are inferred
...
uncertainty from inferring the \AnnualReportFirmMissingCount{} unobserved quarterly firm
counts. Under CPI it declines
...
point at the \AnnualReportSampleStartYear{} slope, but almost no response at the
\AnnualReportSampleEndYear{} slope.
...
\begin{equation}
\hat\pi_t=\bar\alpha\,\pi_{t-1}+(1-\bar\alpha)\,\mathbb{E}_t\pi_{t+1}
+\bar\kt\,x_t+\bar\lambda\,(x_t-\bar\phi x_{t-1}),
...
\begin{equation}
S=-\frac{T}{2}\log\!\big(2\pi\bar\sigma_\eta^2\big)-\frac{1}{2\bar\sigma_\eta^2}\sum_{t=1}^{T}r_t^2 .
\end{equation}
...
We report it alongside a forward-filtered conditional log density (FF-LPD), integrated
over parameter draws from the full-sample posterior. This is a useful in-sample
diagnostic, but it is not prequential: although each state filter excludes the current
inflation observation before scoring it, the parameter draws have seen the full sample.
The displayed heuristic WAIC and PSIS-LOO columns apply their usual transformations to
those filtering densities. They are not standard WAIC or LOO, whose pointwise input is
the likelihood used to fit the posterior, and therefore are not model-selection evidence:
...
$\mathrm{FF\mbox{-}LPD}$ gives $\FitCoreLpdDiff$ and the heuristic WAIC and LOO transforms give $\FitCoreWaicDiff$ and
...
against the forward-filter diagnostic, which is what one expects from a
criterion that rewards a model for the extra latent states it fitted to this same sample.
The numerical ordering survives within these diagnostics: on both CPI indices the time-varying slope
improves every column, while on PPI no column meaningfully separates the models (FF-LPD
...
$\FitCoreWaicDiff$,
which does not depend on importance sampling, is numerically more stable than the LOO
transform but remains nonstandard here. The PPI cells should simply not be read off the
LOO column. None of these columns is a genuine out-of-sample exercise, and the ordering
must not be promoted to a predictive or marginal-likelihood conclusion.
...
same filter with \AnnualReportFirmMissingCount{} of the
\AnnualReportObservationCount{} firm-count rows removed. The estimator is not the issue.
...
$\sigma_N$ down to $\MainSigmaN$ to fit it, which pins $\bar N_t+\hat N_t$ at all
\AnnualReportObservationCount{} quarters
...
\section{Robustness}
\label{sec:robust}
We judge robustness on three completed points: (i) how the sign, interval, and magnitude change
...
follows. The current \AnnualReportRunCount{} runs are full-sample comparisons; until a
separate contiguous
subsample estimation of the corrected revision is done, we do not include pre-2008 or
post-1988 numbers in the conclusions.

\paragraph{Error structure (pending).} A fourth robustness question is whether the
conclusion survives replacing the i.i.d. inflation disturbance with an invertible MA(3)
motivated by overlapping four-quarter inflation. The companion implementation has
smoke-tested cells but not yet the full production run-set, so it is not evidence used
in the conclusions or headline tables.

\section{Conclusion}
Over \AnnualReportSampleStartYear{}--\AnnualReportSampleEndYear{} in the United States,
the low-frequency decline in the number of listed
...
generated automatically from one complete saved run-set, revision
\AnnualReportEstimationRevision. The sample-window correction intentionally advances the
code revision before the long re-estimation is run; until that deferred run completes,
the compiled numerical results remain the last complete run-set. A subsequent report
build selects only the new revision and refreshes the macros, so revisions are never
mixed within one table or claim. Section~\ref{sec:scope} states which failures bear on
the conclusions and which do not.

\paragraph{Settled in the displayed complete run-set.} The positive $\delta$ and flattening $\kt$
...
reports a full-posterior FF-LPD and heuristic WAIC/PSIS-LOO transforms next to the
plug-in score, but none is a genuine prequential criterion. A rolling out-of-sample exercise---re-estimating
...
The run directory uses one stable path per (model, data spec, prior, observation
design), named for that cell and carrying no timestamp.  A superseded result can
therefore remain on disk until that cell is re-estimated.  The report builder admits
only a complete set matching the requested estimation revision and sample signature;
the tables below list exactly that admitted set.  The revision string in each run's
metadata records which vintage of the inputs produced it.
