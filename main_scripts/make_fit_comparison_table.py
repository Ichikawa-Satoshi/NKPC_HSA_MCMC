...
``main_scripts/predictive_comparison.py`` still use parameters drawn from the full-sample
...
main_scripts/predictive_comparison.py) and writes the LaTeX fragment the report inputs.

    python main_scripts/predictive_comparison.py      # first
    python main_scripts/make_fit_comparison_table.py  # then
"""
...
def main() -> None:
    if not SRC.exists():
        raise SystemExit(f"missing {SRC}; run main_scripts/predictive_comparison.py first")
    table = pd.read_csv(SRC)
    if "estimation_revision" not in table or set(table["estimation_revision"].dropna()) != {ESTIMATION_REVISION}:
        raise SystemExit(
            f"{SRC} is stale or has no estimation revision; rerun main_scripts/predictive_comparison.py"
        )
    ...
    macros = ["% Generated by main_scripts/make_fit_comparison_table.py; do not edit by hand."]
    ratios = []
    for price in PRICES:
        sub = main_design[main_design["price"] == price].set_index("model")
